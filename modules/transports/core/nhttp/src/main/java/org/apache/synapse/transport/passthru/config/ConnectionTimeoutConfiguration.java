 ...
 * idle connection to send the next message, salesforce will reset the connection. As a solution to this we decided to introduce
 * a connection eviction mechanism as an improvement. So if a connection is idle for a "connectionIdletime" of time or a
 * connection persisted for more than it's "maximumConnectionLifeSpan" then the connection is removed from the connection pool.
 */
